...
const EscrowCard = (props: {
  escrow: EscrowData;
  handleExchange: (escrow: EscrowData) => void;
}) => {
  const { escrow, handleExchange } = props;
  return (
    <div className="flex bg-violet-300 p-4 rounded-lg w-full justify-between items-stretch mb-1 text-violet-950 font-bold text-sm gap-4">
      ...
      <div className="">
        <button
          onClick={() => {
            handleExchange(escrow);
          }}
          className="bg-violet-600 text-white rounded-lg h-full w-full p-8"
        >
          Exchange
        </button>
      </div>
